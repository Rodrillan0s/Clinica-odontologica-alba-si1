...
export default function Sidebar({ activeMenu, setActiveMenu, user, dataMaster, userRolId, logout }) {
  
  // Buscamos datos extendidos en la lista de usuarios cargada
  const currentUserData = dataMaster.usuarios.find(u => 
    u.id_usuario === user?.id_usuario || u.correo === user?.correo
  ) || user;

  const usernameDisplay = currentUserData?.nombre_usuario || currentUserData?.correo?.split('@')[0] || 'usuario';

  const getRolName = (rolId) => {
      const rolEncontrado = Object.keys(ROLES).find(key => ROLES[key] === Number(rolId));
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-100 flex flex-col hidden md:flex z-20 shadow-sm">
      ...
      <div className="p-6 flex flex-col items-center border-b border-gray-50 bg-gray-50/10">
        <div className={`w-16 h-16 rounded-[1.8rem] flex items-center justify-center text-xl font-black mb-3 shadow-lg ${userRolId === 1 ? 'bg-orange-50 text-orange-600' : 'bg-[#148F77] text-white'}`}>
        ...
        <p className="text-gray-400 text-[10px] font-bold mt-1 tracking-widest lowercase transition-all">@{usernameDisplay}</p>
      ...
      <nav className="flex-1 p-4 space-y-2">
        {menuItems.filter(item => item.roles.includes(Number(userRolId))).map((item) => (
          <button
            key={item.text}
      ...
      <div className="p-6 border-t">
          <button onClick={logout} className="w-full py-4 rounded-2xl text-red-400 font-black text-[10px] uppercase tracking-widest hover:bg-red-50 transition-all">
              Cerrar Sesión
          </button>
      </div>
    </aside>
