...
export default function ModalAsignarOdontologos({ procedure, odontologos, onClose, onSuccess }) {
  ...
  useEffect(() => {
    const fetchAssigned = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`${API_URL}/citas/odontologos-por-procedimiento/${procedure.id}`, {
          headers: {
        ...
        if (data.success) {
          ...
          throw new Error(data.message || "Error al obtener odontólogos asignados.");
        }
      } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/procedimientos/${procedure.id}/odontologos`, {
        method: "POST",
        headers: {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col transform transition-all max-h-[85vh]">
        ...
        <div className="bg-emerald-50 px-6 py-5 border-b border-emerald-100 flex justify-between items-center shrink-0">
          <div>
            <h2 className="text-xl font-black text-[#148F77]">Asignar Odontólogos</h2>
            <p className="text-xs text-emerald-600/70 font-bold mt-1">
              Procedimiento: <span className="text-[#2A5C4D]">{procedure.descripcion}</span>
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              <label className="block text-[11px] font-black text-[#148F77] uppercase tracking-widest mb-2 ml-1 shrink-0">
